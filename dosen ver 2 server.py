...
class BankServer:
    ...
    def handle_client(self, c_socket):
        with c_socket:
            print(f"Connection from {str(self.c_address)}")
    ...
    def add_data(self, c_socket):
        ...
        c_socket.sendall(b'Data berhasil ditambahkan.')
    ...
    def update_data(self, c_socket):
        ...
        try:
            ...
            if 0 < row_index < len(self.data) + 1:
                ...
                c_socket.sendall(b'Data berhasil diperbarui.')
            else:
                c_socket.sendall(b'Nomor baris tidak valid.')
        except ValueError:
            c_socket.sendall(b'Masukkan nomor yang valid.')
    ...
    def delete_data(self, c_socket):
        ...
        try:
            ...
            if 0 < row < len(self.data) + 1:
                ...
                self.data.to_csv(self.filename, index=False)
                c_socket.sendall(b'Data telah dihapus')
            else:
                c_socket.sendall(b'Nomor baris tidak valid.')
        except ValueError:
            c_socket.sendall(b'Masukkan nomor yang valid.')
